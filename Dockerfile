...
RUN apk add --no-cache --virtual .build-deps binutils tar zstd ;\
    ...
    wget -O /tmp/glibc-${GLIBC_VER}.apk ${ALPINE_GLIBC_REPO}/${GLIBC_VER}/glibc-${GLIBC_VER}.apk ;\
    apk add --no-cache /tmp/glibc-${GLIBC_VER}.apk ;\
    wget -O /tmp/glibc-bin-${GLIBC_VER}.apk ${ALPINE_GLIBC_REPO}/${GLIBC_VER}/glibc-bin-${GLIBC_VER}.apk ;\
    apk add --no-cache /tmp/glibc-bin-${GLIBC_VER}.apk ;\
    wget -O /tmp/glibc-i18n-${GLIBC_VER}.apk ${ALPINE_GLIBC_REPO}/${GLIBC_VER}/glibc-i18n-${GLIBC_VER}.apk ;\
    apk add --no-cache /tmp/glibc-i18n-${GLIBC_VER}.apk ;\
    /usr/glibc-compat/bin/localedef --force --inputfile POSIX --charmap UTF-8 "$LANG" || true ;\
    echo "export LANG=$LANG" > /etc/profile.d/locale.sh ;\
    wget -O /tmp/gcc-libs.tar.zst ${GCC_LIBS_URL} ;\
    echo "${GCC_LIBS_SHA256} */tmp/gcc-libs.tar.zst" | sha256sum -c - ;\
    mkdir /tmp/gcc ;\
    tar -I zstd -xvf /tmp/gcc-libs.tar.zst -C /tmp/gcc ;\
    mv /tmp/gcc/usr/lib/libgcc* /tmp/gcc/usr/lib/libstdc++* /usr/glibc-compat/lib ;\
    strip /usr/glibc-compat/lib/libgcc_s.so.* /usr/glibc-compat/lib/libstdc++.so* ;\
    wget -O /tmp/libz.tar.xz ${ZLIB_URL} ;\
